...
**Two database migrations are written but deliberately NOT applied** — held for the merge to `main` so database and code go live together. Until then the live database still lets a signed-out client read every public build (and its checkpoints, tags and likes) directly through PostgREST with the anon key, and the builds row's checkpoint mirror can drift. Files, what each fixes, the dry-run evidence and the merge-day steps: `docs/superpowers/pending-migrations/README.md`. **Do not merge this branch to `main` without applying them.**
