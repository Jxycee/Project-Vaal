...
- **Structural validation** (Slice 3, `plans/2026-09-24-slice3-structural-validation.md`; on `worktree-server-migration`, **not yet merged**). `src/lib/build/validate/` is pure and derives warnings from a checkpoint's live state. Nothing is stored, and nothing blocks. It covers: a two-hander filling its own weapon set's off-hand (the other set stays free), a quiver without a bow, Wand/Sceptre main hands that cannot dual wield, and PoB2's three exceptions (Giant's Blood, Lord of the Wilds, Instruments of Power) read per set. Also items stored in a slot their category never fits, weapon-set passives above 24, and ascendancy points above 8. **The off-hand slot now offers Sceptres, one-hand weapons and Giant's Blood two-handers**, following PoB2's `IsItemValidForSlot`, so PoB import keeps dual-wielded off-hands. The gem sheet shows **raw** reserved Spirit per set; comparing it with Spirit waits for Slice 5. **Not checked:** support-gem uniqueness (sources disagree), a skill's weapon requirement (not in our data), Spirit itself. **The UI is test-grade.** *Verified 2026-09-25:* `e2e/validation.spec.ts` against the live database, 792 unit tests, and a clean build.

- **Items with everything our data backs** (Slice 4, `plans/2026-09-25-slice4-item-affixes.md`; on `worktree-server-migration`, **not yet merged**). A gear or jewel item can carry an optional `craft`: rarity, name, item level, quality, corrupted, implicit and unique roll values, prefixes and suffixes with rolled values, and socketed runes or soul cores. There is **no migration** (`gear_state` is jsonb). Rows saved before Slice 4 read exactly as before. **The write gate keeps a craft byte for byte**; it used to project items to five fields and would have dropped one silently. It checks shape and bounds only, so a resync that renames a mod never makes a build unsavable. Unknown slugs and out-of-range values are *warnings*. `GET /api/wiki/mods?item=&kind=` lists what can roll on a base. It uses PoB2's first-match spawn rule, which reproduces PoB2's own eligible counts on five real bases. **PoB import keeps crafts:** crafted items by GGG mod id (35/35 on the fixture), pasted items by template matching, unique and implicit lines at PoB's range, runes by name, and only what cannot match is reported. Craft warnings cover affix limits (Magic 1+1, Rare 3+3, rare jewels 2+2), one mod per group, eligibility, item level, roll ranges, and runes over PoB2's `socketLimit`. **Not done:** enchantments (not typed in our data), which rune effect applies to which slot (Slice 5), an editor for jewels (their crafts import and validate but are not editable yet), and roll odds (our spawn weights are 0/1 only). **The UI is test-grade.** *Verified 2026-09-25:* `e2e/item-craft.spec.ts` and the extended `e2e/pob-import.spec.ts` against the live database, 942 unit tests, and a clean build whose traces list the mod files for `/builds` and `/api/wiki/mods`.

- **Defence stat engine** (Slice 5, `plans/2026-09-25-slice5-defence-engine.md`; on `worktree-server-migration`, **not yet merged**). A Stats sheet on `/tree`, and behind a tap on the shared page, shows Life, Mana, ES, Armour, Evasion, Str/Dex/Int, the four resistances and Spirit for either weapon set. It sets Spirit against the Spirit that set's gems reserve and warns when reserved exceeds available. It is computed from live state, so nothing is stored. **Stats are typed end to end:** tree nodes come from GGG's `PassiveSkills` table (`npm run sync:stats` → `public/data/tree/0.5.2/node-stats.json`, all 5,150 nodes), base implicits from `Mods` (`implicit-stats.json`), and unique lines by wording against our item mods (`unique-stats.json`, 1,144 of 2,068 lines). All of these use the same stat ids as our mod files, and no display text is parsed where a typed source exists. The arithmetic ports PoB2 (MIT, 2016 David Gowor) with a file:line citation per rule. **Attribute choices** for "+5 to any Attribute" passives are stored (`PassiveState.attributeChoices`, absent when empty), chosen from the node panel, and kept by PoB import. **Campaign progress is derived from the checkpoint's level**, per the user's decision: the resistance penalty and fixed quest rewards follow PoB2's quest table and penalty ladder, and choice rewards are named, not counted. **Every source the numbers do not count is named on the sheet** (runes, untyped unique lines, unmodelled stats, unchosen attributes), and so is every assumption (mid-roll values). **Verification:** PoB's saved stats in the fixture come from an older game version, so they are not the target. Each fixture armour piece instead matches PoB2's current bases through PoB2's item formula, and Dex/Int match PoB's saved values exactly. *Verified 2026-09-25:* the unit suites, `e2e/stats.spec.ts`, and the stat steps in `pob-import.spec.ts` and `sharing.spec.ts` against the live database.

Deliberate deferrals, each recorded with its reason in the relevant spec: build export (import from PoB2 exists) and structured stat priorities.

The campaign resistance penalty is implemented since Slice 5, derived from level (see the engine entry above). Bookmarks have no UI: `toggleBuildBookmark` was **deleted** on 2026-09-23 because nothing called it while a Server Function stays reachable by direct POST. The `build_bookmarks` table and its two policies are untouched, so wiring a UI later needs no migration — see the note left at the foot of `src/app/(dashboard)/builds/actions.ts`.
...
2. **Two-handed occupancy is implementable today.** Each item detail carries `twoHanded`, spot-checked as **100% accurate**: Two Hand Sword (14/14), Two Hand Mace (40/40), Bow (45/45), Crossbow (39/39), Staff (27/27), Warstaff (43/43) all `true`; One Hand Sword (15/15) and Quiver (19/19) all `false`. It was previously recorded as undeterminable — that was true of the slim search *index*, not of the item detail, and the limitation was mis-stated as a data one. **Amended 2026-09-24 (Slice 3): "100% accurate" was true only of the categories sampled.** The flag is wrong for all 37 Talismans (see below) and says `false` for every unique in the merged `Mace` category, some of which are two-handed. Slice 3 decides handedness from `category` instead, and for unique maces from the base named in `uniqueMods.baseType` (all 24 resolve), in `src/lib/build/validate/handedness.ts`.
...
### Resolved 2026-09-24: Talismans are two-handed

All **37 Talisman items carry `twoHanded: false`**, which contradicted `docs/research/poe2/classes-and-ascendancies.md:121`. **The flag is wrong, and the research note is right.** `@poe2-toolkit/item-extractor` derives `twoHanded` from a hardcoded `TWO_HANDED_CLASSES` set that omits `Talisman`, a class added in 0.4.0 (`dist/buildItems.js:23-26, 265`). The same extractor's `CLASS_TAGS` gives Talisman `two_hand_weapon`/`twohand`, and all 31 Talisman bases on disk carry those tags. PoB2 agrees: `src/Modules/Data.lua` `weaponTypeInfo["Talisman"] = { oneHand = false }`. The on-disk flag stays wrong until the extractor is fixed and `npm run sync:wiki` is run. Nothing reads it for handedness now. Detail: `plans/2026-09-24-slice3-structural-validation.md`.
...
| | |
|---|---|
| Unit (vitest, `node` env, no DOM harness) | **942 tests / 60 files** (2026-09-25, after Slice 4) |
| E2E (Playwright, mobile + desktop) | **71 passed, 1 skipped (opt-in network), 0 failed** on the full run after Slice 4 and the tracing fix (2026-09-25). The Slice 3 run before it had 1 unexplained failure, recorded below. Earlier: 67/68 and 68/68 on 2026-09-24 |
...
**One request left pending by the dev server, cause unproven (2026-09-25).** In the Slice 3 full run, `sharing.spec.ts`'s first gear pick timed out. Its `GET /api/wiki/items?slot=boots` **never completed**: status -1, still pending when the trace ended (`1-trace.network` in the test's `trace.zip`). The route answered in the specs before and after it in the same run. The spec passed alone straight afterwards, 47.1s against the failing run's 47.0s. Slice 3 changed nothing on that request's path (boots categories, `loadIndex`, the route) apart from the off-hand category list. `loadIndex` has no path that can leave a promise unresolved. The other awaited call is `getCachedUser()`, a Supabase network call. **Not diagnosed.** If it recurs, capture the server-side timing of that route before changing anything.
