...
- **RUN `npm run sync:wiki` EARLY.** Four fields were being silently discarded by `normalizeSkill` and are now fixed — `qualityStats`, `spellCritChance`, `attackCritChance`, `hoverImage` — but **the fix changes the pipeline, not the files**. No skill record on disk carries any of them until the dataset is regenerated from the GGPK/patch server. That sync is a deliberate, heavier operation nobody has run. Anything wanting gem quality or crit numbers is blocked on it, and the corresponding type fields are optional for exactly that reason.

  A systematic diff of every extractor type against its normalizer (2026-09-23) found `Item` and `Mod` complete — nothing dropped — with all four losses confined to gems. Worth repeating that diff after any `@poe2-toolkit` upgrade, since the failure mode is silent: the data arrives and the mapping simply does not list the field.
