...
### Slice 4 result (2026-09-25)

- **Built:** `src/lib/build/craft.ts` (shape, reader, clamping, summary); the write gate keeping crafts; `src/lib/wiki/modCatalogue.ts` with `GET /api/wiki/mods`; the shared spawn rule in `src/lib/wiki/spawn.ts`; craft warnings in `src/lib/build/validate/affixRules.ts`; client lookups and `useCraftData`; the TEST-GRADE `ItemEditorSheet` opened from a gear row; the craft line on the gear sheet and the shared page; and PoB import keeping crafts (`src/lib/pob/craftText.ts`, `mapCraft.ts`).
- **Changed from the plan, each written up above:** the gate checks shape only and unknown slugs warn. The rune picker lists every SoulCore and the effect-to-slot mapping waits for Slice 5. Pasted items keep a mod at its best roll when its display units differ from its roll units. There is no jewel editor yet.
- **Verified:** 942 unit tests / 60 files. `e2e/item-craft.spec.ts` covers the warning at four prefixes and its clearing at three, the clamp, and every field back after a full reload; its tap scan measured exactly 12 controls. `e2e/pob-import.spec.ts` shows the crossbow as "rare · 6 affixes · 2 runes" and all six affixes in the editor after a reload. The full suite is 71 passed, 1 skipped, 0 failed. The database is back to 1 build / 1 checkpoint / 0 `E2E-`.
- **Tracing:** a clean build lists all 5,267 mods for `/builds` (the importer) and `/api/wiki/mods`. The cloud session's fix, merged here as `74bc1ad03a`, removed the directory-valued `ROOT` constants that made every wiki reader trace every icon. `/api/wiki/items` went from 18,952 traced files to 106, and no trace lists an icon. Checked against the build output, not taken from its commit message.
- **Open for Slice 5:** implicit and unique lines are stored as display-unit values and still need `stat` mapping; affix values are in roll units (crit 311–380), which the editor shows raw; and rune effects need the label→slot table.
