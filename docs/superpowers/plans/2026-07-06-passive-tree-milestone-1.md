...
## Research cross-check (2026-07-06) — corrections incorporated

Studied `poe2-tools/poe2-build-planner` (closest architectural match: TS, ~5,100-node tree, TDD'd allocation engine, weapon-set handling, ascendancy overlay, vendored atlases) and cross-checked the live 0.5.2 data. Validated: direct `x`/`y` positioning, ascendancy recentered to the hub, red(set1)/green(set2)/gold(both) tagging (independent match), `{set1,set2}`↔`.build` equivalence, and that a spatial-index + LOD renderer clears 5,100 nodes (their Canvas2D does; our Pixi is headroom). Corrections now folded into the plan:

- **Weapon set = per-node tagging, not a point pool (DECISION, supersedes earlier "full weapon-set rules").** Enforced pools are **main + ascendancy only**; the weapon set is a per-node tag (1/2/untagged=both) driving the red/green/gold colour, matching GGG's `.build` format. `passive_state` shape is `{ set1, set2, ascendancyNodes }` — set1/set2 encode the tags (both = in both arrays). Stage 3's budget engine drops the weapon-set pool + extension rules; it counts main (unique allocated point-costing nodes) and ascendancy (allocated `ascendancyNodes`).
- **Connection arcs (Stage 1).** Draw an arc around the group centre when both endpoints share `group` + `orbit` (orbit>0), else a straight line — ~1,603 of ~6,074 edges are same-orbit arcs, so straight lines would break every cluster. Parser now carries `group`/`orbit` per node and a `groups` centre map (implemented, verified).
- **Ascendancy render (Stage 1) = central integrated circle (DECISION, supersedes the panel-vs-inline question).** Main tree renders from raw coords (origins ring the hollow centre at r≈1450); the six chosen ascendancy is **recentered to (0,0) and scale-fit to a canonical ~1550-unit diameter**, drawn over the class art frame (`background-<class>.webp`, per-ascendancy frames) in the centre hole; the other clusters aren't drawn. Zoom governs detail (zoomed-out = whole tree, zoomed-in = ascendancy view).
- **Allocation graph (Stage 2).** Keep a nav graph distinct from the render graph, pruning the ascendancy-bridge and cross-class edges, so base-tree BFS can't path into an ascendancy. Ascendancy allocation is its own sub-graph BFS gated by selection + the ascendancy pool.
- **Decoration nodes (Stage 1/2).** Masteries are decorative in 0.5.x (368, zero stats — rendered, not allocatable) and there are ~238 empty proxy connectors (no name/icon; kept in the graph for pathing, not rendered as icons, not allocatable). Parser flags both via `isDecoration` (implemented, verified).
- **Multiple-choice nodes (Stage 2).** 20 in current data; pick-one-of-a-group. Parser carries `isMultipleChoiceOption` + `multipleChoiceParent` (implemented, verified).

**Parser status:** `src/lib/tree/{types,parse}.ts` revised to carry group/orbit + groups map + decoration + multiple-choice + `ascendancyNodes`; `tsc` clean, 20/20 vitest pass (13 fixture + 7 real-data). The Stage 1 pre-task (Context7 Pixi v8 docs) and the arc/ascendancy render are next.
