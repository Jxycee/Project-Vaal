...
**Working state (no build needed) & Save-to-Build (§8.6): not yet built.** The `passive_state` <-> `BuildAllocation` reconciliation this paragraph used to block on is resolved — see §13's entry (2026-08-28) and `src/lib/builds/passiveState.ts`. What's actually left is wiring: localStorage persistence for unsaved tree state, and the Save-to-Build flow itself (create-new vs. add-to-existing, the destructive-replace-warning below) — both real UI work, not an open design question anymore.
...
- [x] ~~Passive-tree `passive_state` schema vs. `@poe2-toolkit`'s runtime allocation shape: reconcile `{set1, set2, ascendancyNodes}` (§6) against the library's `{allocated, weaponSets}` before wiring localStorage working-state persistence or Save-to-Build (§8.5/§8.6)~~ — **resolved 2026-08-28** (this item predates the move to Claude Code and was written without having read `@poe2-toolkit/tree-core`'s actual installed types — done now, see `src/lib/builds/passiveState.ts`): the live schema never actually carries `ascendancyNodes` (checked the deployed table directly — default is `{"set1": [], "set2": []}`, not the three-array shape this doc's §6 still shows), and per `BuildAllocation.weaponSets`'s own doc comment ("Keystones, jewel sockets and ascendancy nodes are always basic") that's correct, not a gap — ascendancy nodes fold into the same shared/basic case as any other node with no weapon-set entry, so the plain two-array `{set1, set2}` shape already fully represents it. `toBuildAllocation`/`fromBuildAllocation` convert between that and tree-core's `{allocated, weaponSets}` losslessly (unit-tested); `resolveClassId` + a documented one-line passthrough for `ascendId` (`AscendancyDef.id` is just the display name already stored in `builds.ascendancy`) round out `BuildAllocation`'s other fields. §6's `ascendancyNodes` example in this doc is now stale — the real shape is `{set1, set2}`, full stop.
...
- [x] ~~Passive-tree point-budget/stat panel UI: allocation works; no point-counting or stat display yet~~ — **resolved 2026-07-30**: `TreeControls`'s mode row now shows `spent/max` next to Main/Set I/Set II (123 shared/basic points — 99 from levelling 2-100 + 24 from quest rewards — and 24 per weapon set, matching the reference tree's budget readout; fixed game constants, not derived from any save data). Stat display (tooltip/info panel) already shipped earlier (§13 above, 2026-07-11).
...
- [x] ~~Passive-tree mobile tooltip UX~~ — **resolved 2026-07-14** (implemented via Claude Code, verified by reading the actual shipped code): touch taps now preview an allocation before committing rather than allocating immediately. Uses `tree-react`'s existing `AllocationPreview`/`preview` prop (confirmed unused before this fix, not a library patch) — a dry-run of `toggleAllocationInMode`/`toggleAscendancyAllocation` (both already side-effect-free) computes what a tap *would* do, renders it as a highlight on the tree, and `NodeInfoPanel` gains an explicit "Allocate"/"Remove" button that commits on press. Touch-only, detected via `matchMedia('(pointer: coarse)')` — desktop's click-to-commit-immediately behavior is untouched, since it already has a free preview via mouse hover. Tapping a different node while one's pending just replaces it, no forced confirm/cancel.
- [x] ~~Passive-tree credit label position~~ — **resolved 2026-07-14**: offset reduced from `+150` to `+80` in `PassiveTree.tsx`'s `creditLabels` memo.
- [x] ~~Abyssal Lich ascendancy (Witch's hidden 4th ascendancy) is missing from the tree's class/ascendancy picker.~~ — **resolved 2026-07-30**: fixed via a new `tree-core` patch, `patches/@poe2-toolkit+tree-core+0.4.1.patch` (`normalizeClass`/`normalizeAscendancy` in `ggg/normalize.js`). Recognizes a named ascendancy with no start node of its own but a resolvable base ascendancy — resolved from the data itself (the override table's *base* node ids each carry their own real `ascendancyId`), not an id-naming convention that might not hold for whatever the next such ascendancy's id looks like. Two new `AscendancyDef` fields: `graphId` (the ascendancy whose actual node graph this one defers to — its borrowed nodes are tagged with the BASE ascendancy's display name, e.g. "Lich", not its own — so `buildAscendancyGraph`/`toggleAscendancyAllocation`/`TreeView`'s `activeAscendancy` prop must resolve through it, or they search for a name matching nothing) and `nodeOverrides` (the 13 base-skill-id → `{name, icon, stats}` overrides, resolved against `raw.skillOverrides`, the same global table already used for the generic attribute nodes). `buildScene` (same patch file) applies the active ascendancy's icon override the same way the existing class-level `overridePairs` icon override already works. `PassiveTree.tsx` resolves `graphAscendancyId` for the graph/rendering call sites and applies the name/stats override in `handleNodeClick`/`handleNodeHover`. Verified: automated test (`src/lib/tree/__tests__/abyssalLich.test.ts`) against the real vendored `data.json` (13 overrides present, 3 headline ones — Steward of Kulemak, Unwilling Offering, Umbral Well — match documented names), plus live in-browser verification (Abyssal Lich now selectable; graph/hub position match Lich exactly; all 13 overridden nodes show the correct alternate name/icon/stats on click; unswapped Lich nodes and plain Lich itself are unaffected).
...
- [x] ~~Passive-tree performance: `@poe2-toolkit/tree-react`'s `TreeView` fully destroys and rebuilds the entire scene graph...~~ — **Tier 1 resolved 2026-07-30 (centre-art image loads no longer trigger a full rebuild); Tier 2 still open (incremental allocation-only rebuild).** Extended `patches/@poe2-toolkit+tree-react+0.7.2.patch` (`TreeView.js`): the image-loading effect previously routed every centre-art image's `onload` (the class portrait + shared ring art — 1-2 per page load) through the full `rebuildRef` (destroying/recreating every one of 1500+ node sprites just to place 1-2 hub sprites). It now routes those specifically through a new, narrow `rebuildCentre` that calls the already-isolated `buildCentre` directly, touching only `centreLayer`; jewel-icon loads (none yet — no gear/Save-to-Build) correctly still use the full rebuild, since a jewel sprite lives in `nodeLayer` alongside everything else. Verified with cache-busted image URLs (to force genuine network loads past the browser's HTTP cache): full-scene-rebuild count on a fresh `/tree` load dropped from **3** (1 initial mount + 2 image-triggered) to **1** (initial mount only — both image loads now take the cheap centre-only path), confirmed by directly instrumenting call counts (React StrictMode's dev-mode double-mount plus an already-warm image cache made the Long Tasks API too noisy for this specific before/after comparison in a dev session). **Tier 2 (an id-indexed sprite map so an ordinary allocation click updates just the changed nodes' tint in place, plus splitting `connLayer` into static/dynamic `Graphics` so only the changed rails redraw) is a substantially larger, riskier rewrite of the node/connection scene-graph lifecycle — deliberately deferred, not attempted, per the original scoping note below that it deserves its own session rather than being forced. Original scoping, still accurate for what's left: `TreeView`'s `rebuild()` still fully destroys and rebuilds the entire scene graph — every one of 1500+ nodes — on every relevant prop change (`scene`, `resources`, `centreSprites`, `activeClassId`, `activeAscendancy`), including an ordinary allocation click. A real fix means patching `rebuild()` to update incrementally instead of wholesale; substantial third-party scene-graph lifecycle rewrite, own scoped session.
