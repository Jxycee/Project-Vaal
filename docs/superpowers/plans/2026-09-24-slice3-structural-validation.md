...
| ~~**Uniques carry no tags and no base.**~~ **Wrong about the base, found 2026-09-24 during Slice 4 research:** 422 of 440 uniques carry `uniqueMods.baseType`, and 419 of those resolve to a base by name. The other 3 resolve once a `{variant:…}` prefix is stripped. All 24 unique maces resolve (13 two-handed, 11 one-handed). The rest of this row was the original claim; see the follow-up task at the foot of this plan. |
| (original) **Uniques carry no tags and no base.** Their `category` is the unique-stash category. For `Mace` (24 uniques) that merges one- and two-handed maces, so **their handedness is unknown to us**. E.g. PoB2 lists Hrimnor's Hymn on Oak Greathammer (2H) and Frostbreath on Slim Mace (1H), while our data gives both category `Mace`, `twoHanded: false`. Unique Bow, Crossbow, Staff and Warstaff are all `twoHanded: true`; unique Talismans are category `Talisman`. | script over `items/*.json`; PoB2 `src/Data/Uniques/mace.lua` |
...
2. ~~**Unique bases.** Knowing a unique's base needs data we do not extract.~~ **Wrong:** `uniqueMods.baseType` already carries it. See the follow-up task below.
