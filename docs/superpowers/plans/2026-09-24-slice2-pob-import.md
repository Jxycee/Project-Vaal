...
| **13 of the build's 20 gems carry a different name in our data than in PoB** — GGG renames gems between patches. Worst case: PoB's "Artillery Ballista" (`SkillGemArtilleryBallista`) is now named **"Siege Ballista"**, and the name "Artillery Ballista" belongs to a *different* gem (`SkillGemRipwireBallista`). A name join would silently swap in the wrong skill; the id join imports the player's own gem under today's name. The report lists every rename as a note. | compared every `<Gem nameSpec>` with our name for the same `gemId`, 2026-09-24 |
...
- [x] **mapTree failure modes:** an unknown node id is dropped and reported by id; an ascendancy node for a **different** ascendancy than the build's is dropped and reported; `WeaponSet1` nodes land in `set1` only, `WeaponSet2` in `set2` only, all others in both; an empty spec yields an empty `PassiveState`, not an error; `attributeOverrides` present → one report entry saying how many attribute choices were dropped. **Fixture:** spec 8 → 124 of 127 kept (116 main-tree, **8** ascendancy — exactly the cap), `15671` reported, and the two start nodes omitted silently (they are implicit in our model, not lost).
- [x] **Start nodes are never imported.** Verified 2026-09-24: PoB's `nodes` includes the class start (`50986` "DUELIST", shared by Duelist and Mercenary) and the ascendancy start (`7120`), while our editor never stores a start node — tree-core's `pathToNode` builds paths while `!sources.has(step)`, excluding the start it paths from. Keeping them would put a node in state the editor never produces, and would push spec 8's ascendancy count from 8 to 9, over the cap. A test pins both omissions.
- [x] **mapGems failure modes:** empty label groups are skipped silently; a group whose first gem is a support (per catalogue `gemType`) is reported and skipped; an unknown `gemId` is reported by `nameSpec`; more than `MAX_SUPPORTS_PER_SKILL` supports → the extras reported by name; a disabled gem is dropped and reported; gem level is clamped to 1–40 and quality to 0–20, each clamp reported; `mainSocketGroup` pointing at a label row or a dropped group → no primary, reported. **Fixture:** 5 loadouts, 20/20 gems resolved, primary = the Artillery Ballista loadout.
- [x] **mapItems failure modes:** `Ring 3` reported, never forced into `ring2`; a second life flask reported; a magic item whose name contains no known base reported; rolled mods, `Variant`, `Rune`, enchant and `Quality` lines counted and reported per item, never silently discarded; an unknown unique name falls back to its base line before reporting. **Fixture:** every slot in the decode findings' slot table maps as listed there.
...
- [x] `ImportSheet.tsx`, opened by an "Import from PoB" button on `/builds`. A textarea, Preview, the report rendered as plain lists (**kept**, **dropped**, **inferred**), a name input, Import. On success, navigate to `/tree?build=<id>`. Portal at `z-40`, 44px controls, marked TEST-GRADE in its header comment.

## Task 10: E2E — the primary verification

- [x] `e2e/pob-import.spec.ts`: paste the **vendored** code (no network dependency in CI), Preview, assert the report states 8 checkpoints, 20/20 gems (13 listed as renamed, e.g. "Artillery Ballista → Siege Ballista"), node `15671` dropped, attribute choices dropped, gear and gems shared across checkpoints, and `Ring 3` unoccupied; Import; on `/tree?build=<id>` assert **8** checkpoints in the sheet, and that the first and last checkpoints come back after a full reload with **different, non-zero** node counts. The tree hook's `allocated` is **main-tree only**, so the expected values are not the raw spec totals (39 and 127): verified on 2026-09-24 against our 0.5.2 tree, spec 1 imports as **35** main-tree nodes and spec 8 as **116**. (Corrected twice before any code: the raw spec totals are 39 and 127; excluding ascendancy nodes gives 36 and 117; and PoB's lists also contain the class start node `50986` and the ascendancy start `7120`, which our editor never stores — see the mapTree start-node rule — giving 35 and 116.) Before hard-coding them, confirm by one manual import that the editor keeps every stored node on seeding rather than re-pathing or dropping disconnected ones — if it does not, that is a finding to record, not a number to adjust until green. Tap-target scan of the sheet with the scanned count asserted. `E2E-` name; `afterAll` cleanup; verify the table afterwards.
- [x] One **opt-in** test (`test.skip` unless `E2E_NETWORK=1`) that imports by pobb.in URL, so the live path is exercisable without making CI depend on a third-party site.
...
### Slice 2 result (2026-09-24)

- **E2E:** `e2e/pob-import.spec.ts` passes against the live database. The preview states 8 checkpoints; 5 skills with 20 gems, 13 of them renamed; 12 items; 2 jewels; node 15671 dropped; attribute choices dropped; gear and gems shared; nothing about Ring 3. The import lands on `/tree?build=<id>`. After full reloads, the first checkpoint seeds **35** main-tree nodes and the last **116**. That confirms the editor keeps every stored node on seeding rather than re-pathing or dropping any. The full suite is 69 passed, 1 skipped (the opt-in network test, which passes with `E2E_NETWORK=1`).
- **The tap-target scan found a real bug:** in the sheet's flex column, a long preview squeezed the Preview button to 22px. The scroll body's children are now `shrink-0`. The scan asserts exactly 3 controls (close, Preview, Import), because `measureTapTargets` counts only buttons and links.
- **Not done here:** real UI design (the sheet is test-grade), Maxroll and poe.ninja success paths (no public PoE2 id found to verify them), attribute choices, and rolled item mods. All of these are reported to the user at import time.
