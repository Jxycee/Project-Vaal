...
// =============================================================================
// PricesClient — all interactive behavior for /prices (search, category
// filters, currency-base switching, league switching). Initial data (default
// league + that league's rows) is fetched server-side by page.tsx and handed
// in as props, so first paint needs no client round-trip. Switching to a
// different league is a rare, user-triggered action and still fetches
// client-side — there's no way to prefetch every league up front.
// =============================================================================
...
export default function PricesClient({
  ...
  // Leagues are discovered once, server-side, at request/build time — the
  // list itself doesn't change within a client session, so it needs no
  // setter (a real season rollover shows up on the next page load).
  ...
  // Load rows for the league whenever it changes. The very first render
  // already has `initialRows` for `initialLeague` from the server, so skip
  // this effect's first run — it would otherwise immediately re-fetch the
  // exact data we already have. Only an actual league switch (a later run)
